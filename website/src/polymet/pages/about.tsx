...
export function About() {
  ...
  const handleImageUpload = async (file: File) => {
    ...
    try {
      ...
      console.log(`[About] Direct uploading ${imageId} to ImageKit...`);
      
      // Use direct client-side upload to ImageKit
      const result = await uploadToImageKitDirectFormData(file, imageId);
      
      if (result.success) {
        console.log(`[About] ✅ Direct upload successful: ${result.resizedUrl}`);
        setTempImageUrl(result.resizedUrl);
      } else {
        console.error(`[About] Direct upload failed: ${result.error}`);
            ...
            {isEditingImage && (
              <div className="space-y-3 p-4 bg-muted/50 border border-border rounded-lg">
                <p className="text-xs text-muted-foreground">
                  Upload an image (up to 25MB) directly to ImageKit or enter an external URL
                </p>
