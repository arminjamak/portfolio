    ...
    if (stored) {
      ...
      for (const project of projects) {
        ...
        for (let j = 0; j < project.history.length; j++) {
          if (
            project.history[j].image &&
            project.history[j].image!.startsWith("indexeddb:")
          ) {
            const imageId = project.history[j].image!.replace("indexeddb:", "");
            console.log(
              `[loadProjects] Loading history image from IndexedDB: ${imageId}`
            );
            const dataUrl = await storageService.getImage(imageId);
            if (dataUrl) {
              console.log(
                `[loadProjects] ✅ History image loaded: ${imageId} (${(dataUrl.length / 1024).toFixed(1)}KB)`
              );
              project.history[j].image = dataUrl;
            } else {
              console.warn(
                `[loadProjects] ⚠️ History image not found in IndexedDB: ${imageId}`
              );
            }
          }
        }
...
const saveProjects = async (projects: Project[]): Promise<void> => {
  ...
  try {
    ...
    for (const project of projectsToSave) {
      ...
      for (let j = 0; j < project.history.length; j++) {
        if (project.history[j].image && isDataUrl(project.history[j].image!)) {
          const imageId = `${project.id}-history-${j}`;
          console.log(
            `[saveProjects] Saving history image to IndexedDB: ${imageId}`
          );
          await storageService.saveImage(imageId, project.history[j].image!);
          project.history[j].image = `indexeddb:${imageId}`;
          console.log(
            `[saveProjects] History image saved successfully: ${imageId}`
          );
        }
      }
    }
